...
async function main() {
    ...
    async function processAnimal(animal: ScrapedAnimal): Promise<{ newHash?: [string, string] }> {
        ...
        try {
            ...
            if (existing) {
                ...
                // (avoids inflating count from failed scraper runs)
    ...
    // This prevents mass-delist when the upstream API is down or schema changed.
    let totalDelisted = 0;
    if (created + updated === 0) {
        ...
        for (const [pfId] of shelters) {
            try {
                ...
                if (delisted.count > 0) {
                    console.log(`   🔄 Delisted ${delisted.count} animals not seen for 48+ hours from ${pfId}`);
                    totalDelisted += delisted.count;
                }
            } catch (err) {
                console.error(`   ⚠ Reconciliation failed for ${pfId}: ${(err as Error).message?.substring(0, 80)}`);
            }
        }
    }

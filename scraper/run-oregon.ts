/**
 * Run Oregon — ODA Animal Shelter Statistics
 *
 * Usage:
 *   npx tsx scraper/run-oregon.ts --dry-run
 *   npx tsx scraper/run-oregon.ts --min-intake 100
 */
import 'dotenv/config';
import { createPrismaClient } from './lib/prisma';
import { getOregonStats } from './adapters/oregon-oda';
...
async function main() {
    ...
    console.log(`🌲  Golden Years Club — Oregon ODA Stats${dryRun ? ' (DRY RUN)' : ''}`);
    console.log(`   Source: Oregon Dept. of Agriculture — SB 328 Annual Report`);
    console.log(`   Year: 2024 | Min intake: ${minIntake}\n`);

    const reports = getOregonStats();
    ...
    for (const r of qualifying) {
        const dbId = `or-oda-${r.facilityName.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 60)}`;
        try {
            ...
            const data: Record<string, any> = {
                ...
                dataYear: 2024, dataSourceName: 'Oregon ODA (SB 328)',
                dataSourceUrl: 'https://www.oregon.gov/oda/programs/AnimalHealth',
            ...
            await prisma.shelter.upsert({
                ...
                create: {
                    id: dbId, name: r.facilityName, county: r.city, state: 'OR',

/**
 * Run Missouri — ACFA Animal Shelter Statistics
 *
 * Usage:
 *   npx tsx scraper/run-missouri.ts --dry-run
 *   npx tsx scraper/run-missouri.ts --min-intake 100
 */
import 'dotenv/config';
import { createPrismaClient } from './lib/prisma';
import { getMissouriStats } from './adapters/missouri-acfa';
...
async function main() {
    ...
    console.log(`🐄  Golden Years Club — Missouri ACFA Stats${dryRun ? ' (DRY RUN)' : ''}`);
    console.log(`   Source: Missouri Dept. of Agriculture — ACFA Program`);
    console.log(`   Year: 2024 | Min intake: ${minIntake}\n`);

    const reports = getMissouriStats();
    ...
    for (const r of qualifying) {
        ...
        try {
            ...
            const data: Record<string, any> = {
                ...
                dataYear: 2024, dataSourceName: 'Missouri ACFA',
                dataSourceUrl: 'https://agriculture.mo.gov/animals/acfa/',

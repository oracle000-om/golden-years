    ...
    async function processAnimal(animal: ScrapedAnimal): Promise<void> {
        ...
        try {
            ...
            if (hasExistingCv && photoUnchanged) { cvSkipped++; }
                ...
                try { cvEstimate = await cvProvider.estimateAge(animal.photoUrl, animal.photoUrls); if (cvEstimate) cvProcessed++; } catch { /* skip */ }
            }
            ...
            const data: Record<string, any> = {
                ...
                intakeReason: animal.intakeReason, intakeReasonDetail: animal.intakeReasonDetail,
                euthScheduledAt: animal.euthScheduledAt, intakeDate: animal.intakeDate,
                notes: animal.notes, lastSeenAt: now,
            };

 ...
 *   npx tsx scraper/run-petango.ts --shard=0 --total-shards=4  # sharded
 */
...
async function main() {
    ...
    console.log(`🐾 Golden Years Club — Petango Sync${dryRun ? ' (DRY RUN)' : ''}${noCv ? ' (NO CV)' : ''}${shard != null ? ` (SHARD ${shard + 1}/${totalShards})` : ''}`);
    ...
    if (!dryRun && !shelterArg) {
        const proceed = await shouldScrape('petango', { shard, minIntervalMs: 6 * 60 * 60 * 1000 });
    ...
    const runId = await startRun('petango', { shard, totalShards, shelterArg });
    ...
    async function processAnimal(animal: ScrapedAnimal): Promise<void> {
        ...
        try {
            ...
            // Generate visual embedding (Phase 2 — Milvus Lite)
            if (embedHelper) {
            ...
            await enqueueFailure('petango', shelterId, animal.intakeId, (err as Error).message);
        }
    }
    ...
    checkScrapeHealth('petango', created + updated, errors, Date.now() - startTime);
    ...
    try {
        ...
        await pool.query(
            `UPDATE scrape_runs SET status = 'FAILED', finished_at = NOW(), error_summary = $1
             WHERE pipeline = 'petango' AND status = 'RUNNING' AND started_at > NOW() - INTERVAL '6 hours'`,
